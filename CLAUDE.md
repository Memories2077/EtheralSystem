...
bun run build
# Example: SERVER_ID=test123 bun run build

# TypeScript compilation (if needed)
bunx tsc --noEmit  # type checking only
```

### Running Individual Components
```bash
# Set SERVER_ID and run the launcher (for generated MCP servers)
SERVER_ID=<server-id> bun src/launcher.ts
...
- **Generation script**: `bun run build` with `SERVER_ID` env var
...
- The file is generated by `bun run build` first
...
- Use `bun` for running TypeScript directly (no separate compilation step)
...
- **No test suite**: The `test` script is placeholder. Use `bun run build` for generation testing.

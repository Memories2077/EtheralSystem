...
| **MetaClaw không có "điểm vào"**     | Proxy chỉ có giá trị nếu nằm dưới _tất cả_ LLM call – nhưng hiện mỗi agent init LLM riêng               |
...
- [x] Tạo `utils/llm_factory.py` trong `langChain-application` – single entry point để lấy LLM
- [x] Sửa `supervisor.py`, `examiner_agent.py`, `generator_agent.py` dùng factory
- [x] Thêm METACLAW vars vào `.env` của `langChain-application`
...
- [x] Bổ sung vòng lặp iteration trong LangGraph graph (conditional edges: `supervisor_final` → `supervisor`)
- [x] Supervisor dùng LLM để quyết định next agent thay vì hardcode sequence (`_ROUTING_SYSTEM_PROMPT` + `bind_tools`)
- [x] Thêm state tracking: `history`, `retry_count`, `current_plan`
  > **File mới:** `my-agent/agents/graph.py` — graph chính thức Phase 2
  > **`langgraph.json`** đã được cập nhật để trỏ về `agents/graph.py:app`

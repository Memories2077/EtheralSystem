...
async function makeJSONPlaceholderRequest<T>(
  url: string,
  options: RequestInit = {},
): Promise<T | null> {
  const headers = {
...
server.registerTool(
  ...
  async ({ id }) => {
    ...
    if (!post) {
      return {
        content: [
          { type: "text", text: `Failed to retrieve post with ID ${id}` },
        ],
      };
    }
...
server.registerTool(
  ...
  async ({ id }) => {
    ...
    if (!user) {
      return {
        content: [
          { type: "text", text: `Failed to retrieve user with ID ${id}` },
        ],
      };
    }
    ...
    if (!comments) {
      return {
        content: [
          {
            type: "text",
            text: `Failed to retrieve comments for post ${postId}`,
          },
        ],
      };
    }
...
server.registerTool(
  ...
  async ({ id }) => {
    ...
    if (!todo) {
      return {
        content: [
          { type: "text", text: `Failed to retrieve todo with ID ${id}` },
        ],
      };
    }
...
async function main() {
  ...
  app.listen(PORT, () => {
    console.log(
      `JSONPlaceholder MCP Server v2.0 running at http://localhost:${PORT}`,
    );

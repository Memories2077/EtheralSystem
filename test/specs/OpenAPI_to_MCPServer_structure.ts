...
class OpenAPIMCPGenerator {
    ...
    private generateToolName(path: string, method: string, operation: PathOperation): string {
    ...
    private generateInputSchema(operation: PathOperation): Record<string, z.ZodTypeAny> {
    ...
    private buildUrl(pathTemplate: string, pathParams: Record<string, any>, queryParams: Record<string, any>): string {
    ...
    public generateTools(): void {
        Object.entries(this.spec.paths).forEach(([path, pathItem]) => {
            Object.entries(pathItem).forEach(([method, operation]) => {
                ...
                const toolDescription = operation.summary || operation.description || `${method.toUpperCase()} ${path}`;
    ...
    public async start(): Promise<void> {
        ...
        app.route('/mcp').all(async (req: Request, res: Response) => {
            console.log(`${req.method} /mcp, session: ${req.headers['mcp-session-id'] || 'No session'}`);
...
export async function createMCPServerFromOpenAPI(specPath: string): Promise<void> {
    try {
